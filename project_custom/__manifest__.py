# -*- coding: utf-8 -*-
{
    'name': 'Odoo Task',
    'version': '1.0',
    'summary': 'Odoo Task',
    'sequence': -1,
    'license': 'LGPL-3',
    'description': """Project Custom""",
    'category': 'Task ',
    'depends': ['base',
                'web',
                'sale',
                'board',
                'custom_employee',
                'base_setup',
                'sales_team',
                'mail',
                'calendar',
                'resource',
                'utm',
                'web_tour',
                'contacts',
                'digest',
                'phone_validation',
                'project',
                'hr_timesheet',
                'hr',
                'stock',
                'website_sale',
                ],

    'data': [
        'security/security.xml',
        # 'security/rec_rules.xml',
        'views/restrict_mail_field.xml',
        'views/settings_view.xml',
        'views/add_approval_stage.xml',
        'security/ir.model.access.csv',
        'views/add_team_to_project.xml',
        'views/project_team_view.xml',
        'views/city_view.xml',
        'views/project_team_member_view.xml',
        'reports/project_task_xlsx_report.xml',
        'wizard/add_team_member_wizard.xml',
        'views/project_task_stage.xml',
        'views/search_filter_inherit.xml',
        'reports/project_task_within_date_range.xml',
        'reports/project_task_report_template.xml',
        'views/project_custom_menu.xml',
        'data/website_menu.xml',
        'data/web_form_template.xml',
        'data/send_reminder.xml',
        'data/mail_template.xml',
        'data/sequance.xml',
        'data/review_template.xml',
        'data/product_listing_template.xml',
        'data/product_product_details.xml',
        'data/extend_shop_product_filter.xml',
        # 'data/extend_shop_product_product.xml',
        # 'data/shop_menu.xml',
        # 'views/stock_picking.xml',
    ],
    'demo': [
        'project_custom/demo/project_demo.xml',
    ],
    'assets': {
        'web.assets_backend': [
            'project_custom/static/src/css/style.css',
            'project_custom/static/src/js/chatter.js',
        ],
    },
    'assets': {
        'web.assets_frontend': [
            'project_custom/static/src/css/product_listing.css',
        ],
    },
    'installable': True,
    'application': True,
}
