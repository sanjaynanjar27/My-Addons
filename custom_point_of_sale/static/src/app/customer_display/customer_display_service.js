...
export class LocalDisplay extends Reactive {
  ...
  isPopupWindowLastStatusOpen() {
    return (
      window.localStorage.getItem("pos-customerdisplay-local-open") === "true"
    );
  }
  async update({ refreshResources = false, closeUI = false } = {}) {
    ...
    displayBody.appendChild(
      container.querySelector(".pos-customer_facing_display"),
    );
...
export class RemoteDisplay extends Reactive {
  ...
  async connect() {
    ...
    try {
      const { status } = await this.hardwareProxy.message("take_control", {
        html,
      });
  ...
  async update({ closeUI = false } = {}) {
    ...
    if (
      ...
      this.hardwareProxy.connectionInfo.status === "connected"
    ) {
