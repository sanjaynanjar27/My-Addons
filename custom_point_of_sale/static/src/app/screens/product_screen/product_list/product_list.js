...
import { ProductCategoryPopup } from "@point_of_sale/app/screens/product_screen/product_info_popup/product_category_popup";
import { ProductSalePopup } from "@point_of_sale/app/screens/product_screen/product_info_popup/product_sale_popup";
...
export class ProductsWidget extends Component {
  ...
  getShowCategoryImages() {
    return (
      ...
      Object.values(this.pos.db.category_by_id).some(
        (category) => category.has_image,
      ) &&
      !this.ui.isSmall
    );
  }
    ...
    return [
      ...
      .map((category) => {
        ...
        const showSeparator =
          !isRootCategory &&
          [
            ...this.pos.db.get_category_ancestors_ids(
              this.pos.selectedCategoryId,
            ),
            this.pos.selectedCategoryId,
  ...
  get productsToDisplay() {
    ...
    list = list.filter(
      (product) => !this.getProductListToNotDisplay().includes(product.id),
    );
  ...
  async onPressEnterKey() {
    ...
    const cleanedProductWord = searchProductWord.replace(
      /;product_tmpl_id:\d+$/,
      "",
    );
    if (result.length > 0) {
      ...
      this.notification.add(
        _t('No more product found for "%s".', cleanedProductWord),
        3000,
      );
    }
    if (this.state.previousSearchWord === searchProductWord) {
  ...
  createNewProducts() {
    window.open(
      "/web#action=point_of_sale.action_client_product_menu",
      "_self",
    );
  }
  ...
  // Added function to get order lines by ids
  async _getSOLines(ids) {
    const so_lines = await this.orm.call("sale.order.line", "read_converted", [
      ids,
    ]);
    console.log("Output from _getSOLines() : ", so_lines);
    return so_lines;
  }
  //  searching product by display_name getting id and filtering sale order lines which contains similar IDs
  async _getFilteredSaleOrders(productName) {
    ...
    // Search for sale orders where any order_line contains the matched product IDs
    const saleOrders = await this.orm.searchRead(
      "sale.order",
      [["order_line.product_id", "in", productIds]],
      [
        "order_line",
        "partner_id",
        "pricelist_id",
    ...
    //    Store values in saleOrders
    for (const saleOrder of saleOrders) {
      const saleLines = await this._getSOLines(saleOrder.order_line);
      saleOrder.order_line = saleLines;
      console.log(saleOrder);
    }
    return saleOrders;
  }
  //   Adding on click function to return orders by product(list)
  async onProductSaleOrderClick(product) {
    try {
      const saleOrders = await this._getFilteredSaleOrders(
        product.display_name,
      ); // Fetch the sale order
      console.log("Sale Orders:", saleOrders);
      const saleOrderLines = saleOrders.flatMap((order) => order.order_line); // Flatten the order_line arrays
      console.log("Sale Order Lines:", saleOrderLines);
      this.popup.add(ProductSalePopup, { saleOrderLines });
    } catch (error) {
      console.error("Error fetching product sales:", error);
    }
  }
  async onProductCategoryClick(product) {
    try {
      const category = await this.pos.getProductCategory(product);
      console.log("Category object:", category);
      this.popup.add(ProductCategoryPopup, { category: category });
    } catch (error) {
