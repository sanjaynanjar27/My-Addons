...
export class Product extends PosModel {
  ...
  isAllowOnlyOneLot() {
    ...
    return (
      this.tracking === "lot" || !productUnit || !productUnit.is_pos_groupable
    );
  }
  /**
   * @returns {string}
  ...
  async openConfigurator({ initQuantity = 1 }) {
    ...
    if (
      attributes.some(
        (attribute) =>
          attribute.values.length > 1 || attribute.values[0].is_custom,
      )
    ) {
    ...
    return {
      ...
      payload: {
        ...
        price_extra: attributes.reduce(
          (acc, attr) => acc + attr.values[0].price_extra,
          0,
        ),
        quantity: 1,
      },
  ...
  isConfigurable() {
    return this.attribute_line_ids.some(
      (id) => id in this.pos.attributes_by_ptal_id,
    );
  }
  ...
  async getAddProductOptions(code) {
    ...
    if (this.isConfigurable()) {
      const { confirmed, payload } = await this.openConfigurator({
    ...
    if (this.to_weight && this.pos.config.iface_electronic_scale) {
      ...
      if (this.isScaleAvailable) {
        ...
        const { confirmed, payload } =
          await this.env.services.pos.showTempScreen("ScaleScreen", {
            product,
          });
        if (confirmed) {
  ...
  isPricelistItemUsable(item, date) {
    ...
    return (
      ...
      (!item.date_start ||
        deserializeDate(item.date_start, { zone: "utc" }) <= date) &&
      (!item.date_end ||
        deserializeDate(item.date_end, { zone: "utc" }) >= date)
    );
  }
...
export class Orderline extends PosModel {
  ...
  init_from_JSON(json) {
    ...
    this.custom_attribute_value_ids = json.custom_attribute_value_ids.map(
      (attr) => {
        if (attr.length > 0) {
          attr = attr[2];
        }
  ...
  can_be_merged_with(orderline) {
    ...
    return (
      ...
      orderline.get_delivery_note() === this.get_delivery_note() &&
  ...
  export_as_JSON() {
    ...
    return {
      ...
      custom_attribute_value_ids: this.custom_attribute_value_ids.map(
        (attr) => [0, 0, attr],
      ),
  ...
  get_display_price_one() {
    ...
    if (this.pos.config.iface_tax_included !== "total") {
      return round_pr(
        price_unit * (1.0 - this.get_discount() / 100.0),
        rounding,
      );
    } else {
  ...
  get_all_prices(qty = this.get_quantity()) {
    var price_unit =
      this.get_unit_price() * (1.0 - this.get_discount() / 100.0);
    var taxtotal = 0;
  ...
  display_discount_policy() {
    return this.order.pricelist
      ? this.order.pricelist.discount_policy
      : "with_discount";
  }
  ...
  getComboTotalPrice() {
    ...
    return allLines.reduce(
      (total, line) => total + line.get_all_prices(1).priceWithTax,
      0,
    );
  }
  getComboTotalPriceWithoutTax() {
    const allLines = this.getAllLinesInCombo();
    return allLines.reduce(
      (total, line) => total + line.get_all_prices(1).priceWithoutTax,
      0,
    );
  }
  ...
  getDisplayData() {
    return {
      ...
      deliveryNote: this.get_delivery_note(),
...
export class Payment extends PosModel {
  ...
  init_from_JSON(json) {
    ...
    this.payment_method =
      this.pos.payment_methods_by_id[json.payment_method_id];
  ...
  handle_payment_response(isPaymentSuccessful) {
    if (isPaymentSuccessful) {
      ...
      this.can_be_reversed =
        this.payment_method.payment_terminal.supports_reversals;
    } else {
...
export class Order extends PosModel {
  setup(_defaultObj, options) {
    ...
    if (options.json) {
      ...
      const linesById = Object.fromEntries(
        this.orderlines.map((l) => [l.id || l.cid, l]),
      );
      for (const line of this.orderlines) {
        line.comboLines = line.combo_line_ids
          ?.filter((id) => linesById[id])
          .map((id) => linesById[id]);
        const combo_parent_id =
          line.combo_parent_id?.[0] || line.combo_parent_id;
        if (combo_parent_id) {
  ...
  getEmailItems() {
    return [_t("the receipt")].concat(
      this.is_to_invoice() ? [_t("the invoice")] : [],
    );
  }
  ...
  init_from_JSON(json) {
    ...
    if (json.fiscal_position_id) {
      ...
      if (fiscal_position) {
        ...
        console.error(
          "ERROR: trying to load a fiscal position not available in the pos",
        );
      }
    }
    ...
    for (var i = 0; i < orderlines.length; i++) {
      ...
      if (
        ...
        this.pos.db.get_product_by_id(orderline.product_id)
      ) {
        this.add_orderline(
          new Orderline(
            { env: this.env },
    ...
    this.lastOrderPrepaChange =
      json.last_order_preparation_change &&
      JSON.parse(json.last_order_preparation_change);
  }
  ...
  export_for_printing() {
    ...
    return {
      orderlines: this.orderlines.map((l) =>
  ...
  getOrderChanges(skipped = false) {
    ...
    for (const orderlineIdx in this.orderlines) {
      ...
      if (
        ...
        this.pos.db.any_of_is_subcategory(product.pos_categ_ids, [
          ...prepaCategoryIds,
        ])
      ) {
  ...
  hasSkippedChanges() {
    return this.orderlines.find((orderline) => orderline.skipChange)
      ? true
      : false;
  }
  hasChangesToPrint() {
    ...
    const has_taxes_included_in_price = tax_ids_array.filter(
      (tax_id) =>
        ...
        (this.pos.taxes_by_id[tax_id].children_tax_ids.length > 0 &&
          this.pos.taxes_by_id[tax_id].children_tax_ids.every(
            (child_tax) => child_tax.price_include,
          )),
    ).length;
  ...
  _isRefundOrder() {
    if (
      ...
      this.orderlines[0].refunded_orderline_id
    ) {
      return true;
    }
    return false;
  ...
  compute_child_lines(comboParentProduct, comboLines, pricelist) {
    ...
    const originalTotal = comboLines.reduce((acc, comboLine) => {
      const originalPrice =
        this.pos.db.combo_by_id[comboLine.combo_id[0]].base_price;
    ...
    for (const comboLine of comboLines) {
      ...
      const totalPriceExtra =
        priceUnit + attributesPriceExtra + comboLine.combo_price;
      combolines.push({
        comboLine: comboLine,
  ...
  stop_electronic_payment() {
    ...
    var line = lines.find(function (line) {
      ...
      return (
        status &&
        !["done", "reversed", "reversing", "pending", "retry"].includes(status)
      );
    });
    if (line) {
    ...
    if (this.pos.config.cash_rounding) {
      ...
      if (!only_cash || (only_cash && last_line_is_cash)) {
        ...
        if (floatIsZero(rounding_applied, this.pos.currency.decimal_places)) {
          ...
          Math.abs(this.get_total_with_tax()) <
  ...
  has_not_valid_rounding() {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (
        !floatIsZero(
          line.amount -
            round_pr(line.amount, this.pos.cash_rounding[0].rounding),
          6,
        )
      ) {
        return line;
  ...
  check_paymentlines_rounding() {
    if (this.pos.config.cash_rounding) {
      ...
      for (var id in this.get_paymentlines()) {
        ...
        var diff = round_pr(
          round_pr(line.amount, cash_rounding) -
            round_pr(line.amount, default_rounding),
          default_rounding,
        );
    ...
    if (newPartner) {
      newPartnerFiscalPosition = newPartner.property_account_position_id
        ? this.pos.fiscal_positions.find(
            (position) =>
              position.id === newPartner.property_account_position_id[0],
          )
        : defaultFiscalPosition;
      newPartnerPricelist =
        this.pos.pricelists.find(
          (pricelist) =>
            pricelist.id === newPartner.property_product_pricelist[0],
        ) || this.pos.default_pricelist;
    } else {
