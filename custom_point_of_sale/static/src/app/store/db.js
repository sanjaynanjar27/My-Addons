...
export class PosDB {
  ...
  remove_categories(categories_id) {
    categories_id = categories_id.filter(
      (cat_id) => cat_id !== this.root_category_id,
    );
    categories_id.forEach((cat_id) => {
      ...
      if (this.category_childs[parent_id]) {
        this._remove_all_occurrences_from_array(
    ...
    for (const cat_id_str in this.category_ancestors) {
      if (removed_categories_id.has(Number(cat_id_str))) {
        ...
        this.category_ancestors[cat_id_str] = this.category_ancestors[
          cat_id_str
        ].filter(
  ...
  addProductIdsToNotDisplay(product_ids) {
    this.product_ids_to_not_display =
      this.product_ids_to_not_display.concat(product_ids);
  }
  ...
  remove_products(products_id) {
    ...
    products_id.forEach((product_id) => {
      ...
      if (product.available_in_pos) {
        const product_search_string = unaccent(
          this._product_search_string(product),
        );
        const all_categ_ids = product.pos_categ_ids.length
          ? product.pos_categ_ids
          : [this.root_category_id];
        ...
        for (const categ_id of all_categ_ids) {
          ...
          const categ_ancestors_id =
            this.get_category_ancestors_ids(categ_id) || [];
  ...
  remove_product_from_category(product_id, product_search_string, categ_id) {
    ...
    if (this.category_search_string[categ_id]) {
      this.category_search_string[categ_id] = this.category_search_string[
        categ_id
      ].replace(product_search_string, "");
    }
    if (!this.category_search_string[categ_id]) {
  ...
  add_packagings(productPackagings) {
    productPackagings?.forEach((productPackaging) => {
      if (productPackaging.product_id[0] in this.product_by_id) {
        this.product_packaging_by_barcode[productPackaging.barcode] =
          productPackaging;
      }
    });
  }
  ...
  add_partners(partners) {
    ...
    for (const id in updated) {
      ...
      for (
        let id = chunkId * CHUNK_SIZE;
        id < (chunkId + 1) * CHUNK_SIZE;
        id++
      ) {
  ...
  get_product_by_barcode(barcode) {
    if (this.product_by_barcode[barcode]) {
      ...
      return this.product_by_id[
        this.product_packaging_by_barcode[barcode].product_id[0]
      ];
    }
    return undefined;
  }
  ...
  is_subcategory(subcategory_id, category_ids) {
    const check = (categ_id) =>
      categ_id && this.is_subcategory(categ_id, category_ids);
    return (
