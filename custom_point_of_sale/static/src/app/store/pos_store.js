...
export class PosStore extends Reactive {
  ...
  async setup(
    env,
    { popup, orm, number_buffer, hardware_proxy, barcode_reader, ui },
  ) {
    this.env = env;
    this.orm = orm;
    this.popup = popup;
    ...
    this.productListView =
      window.localStorage.getItem("productListView") || "grid";
  ...
  async load_product_uom_unit() {
    const uom_id = await this.orm.call(
      "ir.model.data",
  ...
  async load_server_data() {
    const loadedData = await this.orm.silent.call(
      "pos.session",
      "load_pos_data",
      [[odoo.pos_session_id]],
    );
  ...
  _loadPosSession() {
    ...
    this.pos_session.sequence_number = Math.max(
  ...
  _assignApplicableItems(pricelist, correspondingProduct, pricelistItem) {
    ...
    correspondingProduct.applicablePricelistItems[pricelist.id].push(
      pricelistItem,
    );
  }
  ...
  async updateModelsData(models_data) {
    ...
    if (categories) {
      const previous_categories_id = Object.values(this.db.category_by_id).map(
        (c) => c.id,
      );
    ...
    if (products) {
      const previous_products_id = Object.values(this.db.product_by_id).map(
        (p) => p.id,
      );
  ...
  cashierHasPriceControlRights() {
    return (
      !this.config.restrict_price_control ||
  ...
  async sendDraftToServer() {
    const ordersUidsToSync = [...this.ordersToUpdateSet].map(
      (order) => order.uid,
    );
    const ordersToSync = this.db.get_unpaid_orders_to_sync(ordersUidsToSync);
    const ordersResponse = await this._save_to_server(ordersToSync, {
      draft: true,
    });
    ...
    for (const json of jsons) {
      if (
        ...
        try {
          ...
          console.error(
            "There was an error while loading the order",
            json,
            error,
          );
        }
  ...
  async _loadMissingPricelistItems(products) {
    ...
    const product_tmpl_ids = products.map(
      (product) => product.product_tmpl_id[0],
    );
  ...
  _shouldRemoveSelectedOrder(removeSelected) {
    return (
      removeSelected &&
      this.selectedOrder.server_id &&
      !this.selectedOrder.finalized
    );
  }
  ...
  async _addFiscalPositions(ordersJson) {
    ...
    if (fiscalPositionToGet.length > 0) {
      const fiscalPositionJson =
        await this._getFiscalPositionJson(fiscalPositionToGet);
  ...
  async getProductCategory(product, quantity) {
    console.log("From Category Id :::: ", product);
    const order = this.get_order();
    // check back-end method `get_product_info_pos` to see what it returns
    // We do this so it's easier to override the value returned and use it in the component template later
    const productCategory = product.categ_id
      ? product.categ_id[1]
      : "Unknown Category";
    return {
      productCategory,
  ...
  computePriceAfterFp(price, taxes) {
    ...
    if (order && order.fiscal_position) {
      ...
      taxes.forEach((tax) => {
        const line_taxes = this.get_taxes_after_fp(
          [tax.id],
  ...
  push_orders(opts = {}) {
    ...
    return this.pushOrderMutex.exec(() =>
      this._flush_orders(this.db.get_orders(), opts),
    );
  }
  ...
  push_single_order(order) {
    ...
    return this.pushOrderMutex.exec(() =>
      this._flush_orders(
        [this.db.get_order(order_id)],
        order._getOrderOptions(),
      ),
    );
  }
  ...
  set_synch(status, pending) {
    if (
      ["connected", "connecting", "error", "disconnected"].indexOf(status) ===
      -1
    ) {
    ...
    pending =
      ...
      this.db.get_orders().length +
        this.db.get_ids_to_remove_from_server().length;
  ...
  async _save_to_server(orders, options) {
    ...
    try {
      ...
      order_ids_to_sync.forEach((order_id) =>
        this.syncingOrders.delete(order_id),
      );
    }
  }
  ...
  compute_all(
    ...
    taxes.reverse().forEach(function (tax) {
      ...
      if (tax.price_include && cached_tax_amounts.hasOwnProperty(i)) {
        ...
        var tax_amount =
          total_included_checkpoints[i] -
          (base + cumulated_tax_included_amount);
        cumulated_tax_included_amount = 0;
      } else {
        var tax_amount = self._compute_all(
          tax,
          tax_base_amount,
          quantity,
          true,
  ...
  shouldShowCashControl() {
    return (
      this.config.cash_control && this.pos_session.state == "opening_control"
    );
  }
      ...
      (event) => {
        if (event.key === "message" && event.newValue) {
          ...
          if (
            ...
            msg.session == this.pos_session.id
          ) {
  ...
  resetProductScreenSearch() {
    ...
    this.selectedCategoryId =
      (start_category && iface_start_categ_id?.[0]) || 0;
  }
}
