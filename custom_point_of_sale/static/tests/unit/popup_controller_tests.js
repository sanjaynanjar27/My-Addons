...
QUnit.module("unit tests for PopupContainer", {
  async beforeEach() {
    ...
    registry
      .category("services")
      .add("pos_notification", makeService({ add() {} }));
...
QUnit.test(
  "pressing cancel/confirm key should only close the top popup",
  async function (assert) {
    ...
    assert.containsNone(
      fixture,
      ".custom-popup-2",
      "second popup no longer displayed",
    );
    assert.containsOnce(
      fixture,
      ".custom-popup-1",
      "first popup is displayed again",
    );
