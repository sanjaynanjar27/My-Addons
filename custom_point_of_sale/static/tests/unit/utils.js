...
registry
  ...
  .add("pos.session/load_pos_data", async function (route, args) {
    return {
      "res.company": {
        id: 1,
      ...
      "product.pricelist": [
        ...(this.models["product.pricelist"]?.records || []),
      ],
      "pos.payment.method": [
        ...(this.models["pos.payment.method"]?.records || []),
      ],
...
registry
  .category("mock_server")
  .add(
    "pos.session/get_pos_ui_product_product_by_params",
...
registry
  ...
  .add(
    ...
    async function (route, { args: [model, xmlId] }) {
